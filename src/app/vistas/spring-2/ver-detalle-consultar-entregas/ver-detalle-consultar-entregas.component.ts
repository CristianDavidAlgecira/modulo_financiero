...
@Component({
  ...
  standalone: true,
  imports: [ProgressSpinnerModule, PrimaryButtonComponent, NgIf],
...
export class VerDetalleConsultarEntregasComponent implements OnInit {
  ...
  constructor(private router: Router, private apiService: ApiService, private apiMuvService: ApiMuvService,) {
  ...
  loadOptions(): void {
    ...
    this.apiMuvService.getDetallesByNIT(this.datosState.nit).subscribe((response: any) => {
